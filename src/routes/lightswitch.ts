...
const getStatusResponse = (user: User, clientAllowedActions: string[]) => ({
    serviceInstanceId: "fortnite", status: "UP", message: "Nexus is up.", maintenanceUri: "https://discord.gg/nexusfn",
...
app.get("/lightswitch/api/service/bulk/status", wrapRoute([verifyToken], async (c) => {
    const user = await getAuthUser(c);
    let clientAllowedActions = user?.banned ? ["NONE"] : ["PLAY", "DOWNLOAD"];
    return c.json([getStatusResponse(user!, clientAllowedActions)]);
}));

app.get("/fortnite/api/version", (c) => c.json({
    app: "fortnite",
    ...
    branch: `Release-${c.memory.build}`, modules: {
