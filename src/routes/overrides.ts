...
import app from "..";
import { getClientFromContext, validateClientAndScope } from "../utils/clientauth";
...
const validateClient = (c: Context) => {
    if(c.req.raw.headers.get("bypass")) return true;
    const client = getClientFromContext(c);
    if (!client || !validateClientAndScope(client?.id, client?.secret)) {
        return false;
    }
    return true;
}
...
app.get("/api/nexus/getoverrides", (c) => {
    if (!validateClient(c)) return c.sendError(nexus.authentication.authenticationFailed.withMessage("Invalid client"));
    return c.json({ status: "success", hotfixOverrides });
});

app.get("/api/nexus/getoverrides/:filename", (c) => {
    if (!validateClient(c)) return c.sendError(nexus.authentication.authenticationFailed.withMessage("Invalid client"));
...
app.get("/api/nexus/clearoverrides", (c) => {
    if (!validateClient(c)) return c.sendError(nexus.authentication.authenticationFailed.withMessage("Invalid client"));
    hotfixOverrides.splice(0, hotfixOverrides.length);
    return c.json({ status: "success" });
});

app.get("/api/nexus/clearoverrides/:filename", (c) => {
    if (!validateClient(c)) return c.sendError(nexus.authentication.authenticationFailed.withMessage("Invalid client"));
    const filename = c.req.param("filename");
